...
void	step(void)
{
	...
	{
		plr()->stepX = 1;
		plr()->sideDist.x = ((double)plr()->mapX + 1.0 - plr()->pos.x) * plr()->deltaDist.x;
	}
	if (plr()->rayDir.y < 0)
	{
		plr()->stepY = -1;
		plr()->sideDist.y = (plr()->pos.y - (double)plr()->mapY) * plr()->deltaDist.y;
	}
	else
	{
		plr()->stepY = 1;
		plr()->sideDist.y = ((double)plr()->mapY + 1.0 - plr()->pos.y) * plr()->deltaDist.y;
	}
}
